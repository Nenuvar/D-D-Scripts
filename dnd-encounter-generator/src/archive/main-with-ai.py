...
import re
import requests
from utils.text_ai import generate_ai_text
...
def generate_encounter(monsters, max_xp, config_file=None, difficulty=None):
    """
    ...
    return encounter, selected_environment, main_monster

api_key = "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"
...
def generate_ai_text(prompt, messages=None):
    if messages is None:
        messages = []
...
def main():
    ...
    while True:
        ...
        difficulty_label = {"1": "Easy", "2": "Medium", "3": "Hard", "4": "Deadly"}
        ...
        encounter, environment, main_monster = generate_encounter(filtered_monsters, max_xp, config_file, difficulty)
    ...
    if encounter:
        encounter_names = ", ".join([m.get("name", "Unknown") for m in encounter])
        main_monster_name = main_monster.get("name", "Unknown") if main_monster else "Unknown"
        prompt = (
            f"Create a dramatic D&D 5e encounter introduction. "
            f"Difficulty: {difficulty_label.get(difficulty, 'Unknown')}. "
            f"Environment: {environment}. "
            f"Main monster: {main_monster_name}. "
            f"Other monsters: {encounter_names}."
        )
        print("\n✨ Encounter Flavor Text:")
        print(generate_ai_text(prompt))

    # Save the encounter to a Markdown file
    if not folder_path:
        print("⚠️ No save folder path set. Please set it using the -f or --folder option.")
        return
